...
class Track {
    ...
    async getTrackById(id) {
        const track = await MongooseTrackModel.findOne({ _id: id });
        return track;
    }
    ...
    async updateTrack(id) {
        const validationHandler = new ValidationHandler(this.request, this.response);
        try {
            await MongooseTrackModel.update({_id : mongoose.Types.ObjectId(id)}, {
                $set : {
    ...
    async deleteTrack(id) {
        await MongooseTrackModel.findOneAndRemove({_id : mongoose.Types.ObjectId(id)});
    }
}
